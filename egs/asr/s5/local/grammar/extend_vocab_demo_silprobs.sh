...
stage=0
run_g2p=true  # set this to true to run the g2p stuff, it's slow so
...
out_dir=$1
tree_dir=${out_dir}/exp/tdnn_vwm_sp
#tree_dir=${outdir}/exp/tri6_7d_tree_sp
lang_base=${out_dir}/data/lang_basevocab
lang_ext=${out_dir}/data/lang_extvocab
dict=${out_dir}/data/local/dict
...
if [ $stage -le 4 ]; then
  ...
  awk -v w=${out_dir}/data/lang/words.txt 'BEGIN{while(getline <w) seen[$1] = $1} {for(n=2;n<=NF;n++) if(!($n in seen)) oov[$n] = 1}
                                END{ for(k in oov) print k;}' < ${out_dir}/data/test/text > $tree_dir/extvocab_lexicon/words
...
EOF
  ...
  [ -f ${out_dir}/data/lang_extvocab/G.fst ] && rm ${out_dir}/data/lang_extvocab/G.fst
  utils/lang/extend_lang.sh  \
	  ${out_dir}/data/lang_basevocab \
	  $tree_dir/extvocab_lexicon/lexiconp.txt  \
	  ${out_dir}/data/lang_extvocab
fi
...
#    exp/chain/tree_sp/extvocab_combined data/dev_clean_2_hires exp/chain/tdnn1h_sp/decode_tgsmall_dev_clean_2_ev_comb
	steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
		...
		$tree_dir/extvocab_combined \
		$outdir/data/test_hires_online \
		$dir/decode_test_comb || exit 1;
...
if [ $stage -le 9 ]; then
  echo "$0: stage 9"
  steps/nnet3/decode_grammar.sh --acwt 1.0 --post-decode-acwt 10.0 \
	 --nj 38 --cmd "$decode_cmd" \
	 --online-ivector-dir $outdir/exp/nnet3/ivectors_test \
	 $tree_dir/extvocab_combined \
	 $outdir/data/test_hires_online \
	 $dir/decode_test_comb_gra
