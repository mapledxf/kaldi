...
outdir=$1
gmm_dir=$outdir/exp/${gmm}
ali_dir=$outdir/exp/${gmm}_sp_ali
...
if [ $stage -le 3 ]; then
  ...
  for datadir in ${train_set}_sp $test_set; do
    steps/make_mfcc_pitch$online_affix.sh --nj 10 --mfcc-config conf/mfcc_hires.conf \
      --cmd "$train_cmd" \
      $outdir/data/${datadir}_hires$online_affix \
      $outdir/exp/make_hires/$datadir $mfccdir || exit 1;
    steps/compute_cmvn_stats.sh \
      $outdir/data/${datadir}_hires$online_affix \
      $outdir/exp/make_hires/$datadir $mfccdir || exit 1;
    utils/fix_data_dir.sh \
...
if [ $stage -le 4 ]; then
  ...
  mkdir -p $outdir/exp/nnet3${nnet3_affix}/diag_ubm
  temp_data_root=$outdir/exp/nnet3${nnet3_affix}/diag_ubm

  num_utts_total=$(wc -l <$outdir/data/${train_set}_sp_hires_nopitch/utt2spk)
  num_utts=$[$num_utts_total/4]
  utils/data/subset_data_dir.sh \
     $outdir/data/${train_set}_sp_hires_nopitch \
     $num_utts \
  ...
  steps/online/nnet2/get_pca_transform.sh --cmd "$train_cmd" \
      ...
      --max-utts 10000 --subsample 2 \
       ${temp_data_root}/${train_set}_sp_hires_nopitch_subset \
       $outdir/exp/nnet3${nnet3_affix}/pca_transform
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
    ...
    $outdir/exp/nnet3${nnet3_affix}/pca_transform \
    $outdir/exp/nnet3${nnet3_affix}/diag_ubm
fi
...
if [ $stage -le 5 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
     $outdir/data/${train_set}_sp_hires_nopitch \
     $outdir/exp/nnet3${nnet3_affix}/diag_ubm \
     $outdir/exp/nnet3${nnet3_affix}/extractor || exit 1;
fi
...
if [ $stage -le 6 ]; then
  ...
  ivectordir=$outdir/exp/nnet3${nnet3_affix}/ivectors_${train_set}
  ...
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    $outdir/data/${train_set}_hires_nopitch ${temp_data_root}/${train_set}_sp_hires_nopitch_max2
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 30 \
    ${temp_data_root}/${train_set}_sp_hires_nopitch_max2 \
    $outdir/exp/nnet3${nnet3_affix}/extractor $ivectordir

  # Also extract iVectors for the test data, but in this case we don't need the speed
  # perturbation (sp).
  for data in $test_set; do
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 8 \
      $outdir/data/${data}_hires_nopitch \
      $outdir/exp/nnet3${nnet3_affix}/extractor \
