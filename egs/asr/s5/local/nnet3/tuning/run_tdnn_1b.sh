...
out_dir=/home/data/xfding/asr-noisy-48h/output
dir=$out_dir/exp/nnet3/tdnn_sp${affix:+_$affix}
gmm_dir=$out_dir/exp/tri3
train_set=train
ali_dir=${gmm_dir}_ali
graph_dir=${gmm_dir}/graph

if [ $stage -le 6 ]; then
  mfccdir=$out_dir/mfcc_hires
  for datadir in ${train_set}; do
    utils/copy_data_dir.sh $out_dir/data/${datadir} $out_dir/data/${datadir}_hires
    utils/data/perturb_data_dir_volume.sh $out_dir/data/${datadir}_hires || exit 1;
    steps/make_mfcc_pitch.sh --mfcc-config conf/mfcc_hires.conf --pitch-config conf/pitch.conf \
      --nj $nj $out_dir/data/${datadir}_hires $out_dir/exp/make_mfcc/ ${mfccdir}
  done
fi
...
if [ $stage -le 7 ]; then
  ...
  input_dim=$(feat-to-dim scp:$out_dir/data/${train_set}_hires/feats.scp -)  
...
if [ $stage -le 8 ]; then
  ...
  steps/nnet3/train_dnn.py --stage=$train_stage \
    ...
    --use-gpu true \
    --feat-dir=$out_dir/data/${train_set}_hires \
    --ali-dir $ali_dir \
    --use-gpu wait \
    --lang $out_dir/data/lang \
...
#    num_jobs=`cat $out_dir/data/${decode_set}_hires/utt2spk|cut -d' ' -f2|sort -u|wc -l`
#    decode_dir=${dir}/decode_$decode_set
#    steps/nnet3/decode.sh --nj $num_jobs --cmd "$decode_cmd" \
#       $graph_dir $out_dir/data/${decode_set}_hires $decode_dir || exit 1;
#  done
#fi
