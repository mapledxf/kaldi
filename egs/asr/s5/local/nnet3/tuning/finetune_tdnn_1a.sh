...
data_set=finetune
data_dir=$out_dir/data/${data_set}
ali_dir=$out_dir/exp/${data_set}_ali
src_dir=$out_dir/exp/nnet3/tdnn_sp
...
if [ $stage -le 1 ]; then
  ...
  steps/make_mfcc.sh \
    ...
    ${data_dir} $out_dir/exp/make_mfcc/${data_set} $out_dir/mfcc
  steps/compute_cmvn_stats.sh ${data_dir} $out_dir/exp/make_mfcc/${data_set} $out_dir/mfcc || exit 1;

  utils/fix_data_dir.sh ${data_dir} || exit 1;
  steps/align_si.sh --cmd "$train_cmd" --nj ${nj} ${data_dir} $out_dir/data/lang $out_dir/exp/tri3 ${ali_dir}
  ...
  steps/make_mfcc.sh \
    ...
    ${data_dir}_hires $out_dir/exp/make_mfcc/${data_set}_hires $out_dir/mfcc_hires
  steps/compute_cmvn_stats.sh ${data_dir}_hires $out_dir/exp/make_mfcc/${data_set}_hires $out_dir/mfcc_hires
fi
