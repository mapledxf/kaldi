...
dir=$outdir/exp/chain/tdnn_1b${affix:+_$affix}_sp
train_set=train
ali_dir=$outdir/exp/tri5_ali
treedir=$outdir/exp/chain/tri6_cd_tree_sp
lang=$outdir/data/lang_chain

if [ $stage -le 5 ]; then
  echo "$0: train mfcc"
  mfccdir=$outdir/mfcc_hires
  for datadir in ${train_set} ; do
  	utils/copy_data_dir.sh \
		$outdir/data/${datadir} \
		$outdir/data/${datadir}_hires
	utils/data/perturb_data_dir_volume.sh \
		$outdir/data/${datadir}_hires || exit 1;
...
if [ $stage -le 6 ]; then
  ...
  mkdir -p $outdir/exp/chain/diag_ubm_${affix}
  temp_data_root=$outdir/exp/chain/diag_ubm_${affix}

  num_utts_total=$(wc -l < $outdir/data/${train_set}_hires_nopitch/utt2spk)
  num_utts=$[$num_utts_total/4]
  utils/data/subset_data_dir.sh \
	  $outdir/data/${train_set}_hires_nopitch \
  ...
  steps/online/nnet2/get_pca_transform.sh --cmd "$train_cmd" \
    ...
    $outdir/exp/chain/pca_transform_${affix}
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj $nj \
    ...
    $outdir/exp/chain/pca_transform_${affix} \
    $outdir/exp/chain/diag_ubm_${affix}

  echo "$0: training the iVector extractor"
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj $nj \
    $outdir/data/${train_set}_hires_nopitch \
    $outdir/exp/chain/diag_ubm_${affix} \
    $outdir/exp/chain/extractor_${affix} || exit 1;

  for datadir in ${train_set} ; do
    steps/online/nnet2/copy_data_dir.sh --utts-per-spk-max 2 \
	    $outdir/data/${datadir}_hires_nopitch \
	    $outdir/data/${datadir}_hires_nopitch_max2
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
      $outdir/data/${datadir}_hires_nopitch_max2 \
      $outdir/exp/chain/extractor_${affix} \
      $outdir/exp/chain/ivectors_${datadir}_${affix} || exit 1;
  done
fi
...
if [ $stage -le 7 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" \
	  ...
	  $outdir/data/lang \
	  $outdir/exp/tri5 \
	  $outdir/exp/tri6_sp_lats
  rm $outdir/exp/tri6_sp_lats/fsts.*.gz # save space
fi
...
if [ $stage -le 10 ]; then
  ...
  feat_dim=$(feat-to-dim scp:${outdir}/data/${train_set}_hires/feats.scp -)
...
if [ $stage -le 11 ]; then
  ...
  steps/nnet3/chain/train.py --stage $train_stage \
    --cmd "$decode_cmd" \
    --feat.online-ivector-dir $outdir/exp/chain/ivectors_${train_set}_${affix} \
    ...
    --feat-dir $outdir/data/${train_set}_hires \
    --use-gpu wait \
    --tree-dir $treedir \
    --lat-dir $outdir/exp/tri6_sp_lats \
    --dir $dir  || exit 1;
fi
...
if [ $stage -le 12 ]; then
  ...
  utils/mkgraph.sh --self-loop-scale 1.0 \
	  $outdir/data/lang_test $dir $dir/graph
fi
