...
dir=$out_dir/exp/chain${nnet3_affix}/cnn_tdnn${affix}_sp
...
if [ $stage -le 17 ]; then
		...
		(
			...
			steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
				...
				--online-ivector-dir \
				$out_dir/exp/nnet3${nnet3_affix}/ivectors_${data}_hires \
				$graph_dir \
				$out_dir/data/${data}/test_hires \
				${dir}/decode_${data}_tg || exit 1
		) || touch $dir/.error &
		done
		wait
...
if $test_online_decoding && [ $stage -le 18 ]; then
	...
	steps/online/nnet3/prepare_online_decoding.sh \
		...
		$lang \
		$out_dir/exp/nnet3${nnet3_affix}/extractor ${dir} ${dir}_online
        if $test_enable; then
		rm $dir/.error 2>/dev/null || true
		for data in $test_sets; do
		(
			...
			steps/online/nnet3/decode.sh \
				...
				$graph_dir \
				$out_dir/data/${data}/test ${dir}_online/decode_${data}_tg || exit 1
		) || touch $dir/.error &
		done
		wait
		[ -f $dir/.error ] && echo "$0: there was a problem while decoding" && exit 1
