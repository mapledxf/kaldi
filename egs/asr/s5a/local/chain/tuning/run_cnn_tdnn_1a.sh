...
tree_dir=$out_dir/exp/chain${nnet3_affix}/tree_sp${tree_affix:+_$tree_affix}
lang=$out_dir/data/lang_chain
lat_dir=$out_dir/exp/chain${nnet3_affix}/${gmm}_${train_set}_sp_lats
dir=$out_dir/exp/chain${nnet3_affix}/tdnn${affix:+_$affix}_sp
train_data_dir=$out_dir/data/${train_set}_sp_hires
lores_train_data_dir=$out_dir/data/${train_set}_sp
train_ivector_dir=$out_dir/exp/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires
...
if [ $stage -le 32 ]; then
		...
		(
      			steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
        			...
        			--online-ivector-dir $out_dir/exp/nnet3${nnet3_affix}/ivectors_${decode_set}_hires \
        			$graph_dir \
				$out_dir/data/${decode_set}/test_hires $dir/decode_${decode_set}_tg || exit 1
    		) || touch $dir/.error &
  		done
  		wait
	...
	steps/online/nnet3/prepare_online_decoding.sh \
		...
		(
			...
			steps/online/nnet3/decode.sh \
				...
				$graph_dir \
				$out_dir/data/${data}/test \
				${dir}_online/decode_${data}_tg || exit 1
 		) || touch $dir/.error &
		done
		wait
  		[ -f $dir/.error ] && echo "$0: there was a problem while decoding" && exit 1
...
echo "$0: online decoding test set complete";
