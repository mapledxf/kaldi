...
if [ $stage -le 1 ]; then
	...
	steps/cleanup/clean_and_segment_data.sh --stage $cleanup_stage --nj $nj --cmd "$train_cmd" \
		$data $out_dir/data/lang $srcdir $dir $cleaned_data
fi

if [ $stage -le 2 ]; then
	steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
		$cleaned_data $out_dir/data/lang $srcdir ${srcdir}_ali_${cleanup_affix}
fi

if [ $stage -le 3 ]; then
	steps/train_sat.sh --cmd "$train_cmd" \
		12000 190000 $cleaned_data $out_dir/data/lang ${srcdir}_ali_${cleanup_affix} ${cleaned_dir}
fi

if [ $stage -le 4 ]; then
	# Test with the models trained on cleaned-up data.
	utils/mkgraph.sh $out_dir/data/lang_combined_tg ${cleaned_dir} ${cleaned_dir}/graph_tg
