...
def is_number(s):
    ...
    if s.count('.') == 1:
        ...
        left = s
        right_is_digit = True
    if left.isdigit() and right_is_digit:
        return True
    elif left.startswith('-') and left.count('-') == 1 and right_is_digit:
            lleft = left.split('-')[-1]
            if lleft.isdigit():
                return True
    return False

def num2chinese(num, big=False, simp=True, o=False, twoalt=False, unit=True):
    """
    ...
    if unit:
        ...
        result.append(''.join(c_basic[int(ch)] for ch in nd))
    return ''.join(result)
